defmodule Integration.TokenProvider.EncryptedRedisTokenCacheTest do
  ...
  describe "logs on token encryption failure while setting token" do
    test "malformed token" do
      log =
        capture_log(fn ->
          EncryptedRedisTokenCache.set_token_for(@test_client, @test_audience, <<0x80>>)
        end)
    ...
    test "wrong cache_encryption_key" do
      env_to_restore = Application.fetch_env!(:prima_auth0_ex, @test_client)
      ...
      log =
        capture_log(fn ->
          EncryptedRedisTokenCache.set_token_for(@test_client, @test_audience, sample_token())
        end)
      ...
      Application.put_env(:prima_auth0_ex, @test_client, env_to_restore)
    end
  end
  ...
  test "returns error when persisted tokens are invalid and could not be decrypted" do
    ...
    assert {:error, _} = EncryptedRedisTokenCache.get_token_for(@test_client, @test_audience)
  end
  ...
  defp namespace, do: Application.fetch_env!(:prima_auth0_ex, @test_client)[:cache_namespace]
end
